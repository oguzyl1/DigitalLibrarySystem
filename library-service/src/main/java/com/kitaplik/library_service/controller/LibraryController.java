...
public class LibraryController {
    ...
    @GetMapping("{id}")
    public ResponseEntity<LibraryDto> getLibraryById(@PathVariable(value = "id") String id){
        return ResponseEntity.ok(libraryService.getAllBooksInLibraryById(id));
    }

    @PostMapping
    public ResponseEntity<LibraryDto> createLibrary(){
        logger.info("Kutuphane --" + environment.getProperty("local.server.port") +"-- port numarasi ile olusturuldu");
